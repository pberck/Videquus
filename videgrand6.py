...
#python3 videgrand5.py  -H -c 11 --martingale 24 -b
#python3 videgrand5.py  -H -c 11 --martingale 24 --ref_group day12 -b --train_start_dt "2018-08-01 22:00" --train_end_dt "2018-08-02 20:00"
#
...
parser.add_argument( "--martingale", type=int, default=15,
                     help='w_martingale parameter for Cosmo' )
...
class RSERIES(TSERIES):
    ...
    def generate_samples(self, cam=11, rs="300s"):
        ...
        for x in self.dt_index:
            ...
            elif x.date != 8 and x.hour == 4: # more movement 4 till 5
                yield x, [0.8 * np.random.random_sample()/4]
            else:
...
def plot(results_df, seq, fn_str=None, xtra=None):
    ...
    colours = np.where(results_df["pvalue"].values < 0.1, 'g', 'g') # these can be different
    ax1.scatter( results_df["dt"].values, results_df["pvalue"].values,
                 alpha=0.25, marker=".", color=colours, label="p-value" ) #, s=4 )
